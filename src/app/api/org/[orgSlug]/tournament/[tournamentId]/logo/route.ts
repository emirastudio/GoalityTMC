...
export async function POST(
  ...
  if (tournament.logoUrl) {
    const oldPath = path.join(process.cwd(), "public", tournament.logoUrl);
    await unlink(oldPath).catch(() => {});
  }

  const ext = file.name.split(".").pop() ?? "png";
  const filename = `tournament-logo-${tid}-${Date.now()}.${ext}`;
  ...
  const logoUrl = `/uploads/badges/${filename}`;
  await db.update(tournaments).set({ logoUrl }).where(eq(tournaments.id, tid));
