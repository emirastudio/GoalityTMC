...
async function getListing(listingId: number, organizationId: number) {
  const [listing] = await db.select({ id: listingTournaments.id, photos: listingTournaments.photos })
    .from(listingTournaments)
    .where(and(eq(listingTournaments.id, listingId), eq(listingTournaments.organizationId, organizationId)))
    .limit(1);
  return listing ?? null;
}
...
export async function POST(
  ...
  const listing = await getListing(Number(listingId), organization.id);
  if (!listing) return NextResponse.json({ error: "Listing not found" }, { status: 404 });

  const currentPhotos: string[] = JSON.parse(listing.photos ?? "[]");
  if (currentPhotos.length >= MAX_PHOTOS) return NextResponse.json({ error: `Maximum ${MAX_PHOTOS} photos allowed` }, { status: 400 });
  ...
  const filename = `photo-${orgSlug}-${listingId}-${Date.now()}.${ext}`;
  ...
  await db.update(listingTournaments)
    ...
    .where(eq(listingTournaments.id, listing.id));
