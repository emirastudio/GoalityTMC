...
const MOCK_HOTELS = [
  { name: "Hilton Tallinn Park", stars: 5, address: "A. Lauteri 3", price: "от 120 €/ночь", phone: "+372 630 0000", url: "#" },
  { name: "Nordic Hotel Forum", stars: 4, address: "Viru väljak 3", price: "от 85 €/ночь", phone: "+372 622 9900", url: "#" },
  { name: "Sokos Hotel Viru", stars: 4, address: "Viru väljak 4", price: "от 95 €/ночь", phone: "+372 680 9300", url: "#" },
];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl p-5" style={{ background: "var(--cat-card-bg)", border: "1px solid var(--cat-card-border)" }}>
        <p className="text-[10px] font-bold uppercase tracking-widest mb-4" style={{ color: "var(--cat-text-muted)" }}>Игровые поля</p>
        <div className="space-y-3">
          {MOCK_FIELDS.map((f, i) => (
            <div key={i} className="flex items-start gap-3 p-4 rounded-xl" style={{ background: "var(--cat-tag-bg)", border: "1px solid var(--cat-tag-border)" }}>
              <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 text-[12px] font-black" style={{ background: brand + "15", color: brand }}>
                {i + 1}
              </div>
              ...
              <a href={f.mapUrl} className="text-[11px] font-medium flex items-center gap-1 shrink-0" style={{ color: brand }}>
      ...
      {/* Hotels */}
      <div className="rounded-2xl p-5" style={{ background: "var(--cat-card-bg)", border: "1px solid var(--cat-card-border)" }}>
        <p className="text-[10px] font-bold uppercase tracking-widest mb-4" style={{ color: "var(--cat-text-muted)" }}>Размещение</p>
        <div className="space-y-3">
          {MOCK_HOTELS.map((h, i) => (
            <div key={i} className="flex items-center gap-3 p-4 rounded-xl" style={{ background: "var(--cat-tag-bg)", border: "1px solid var(--cat-tag-border)" }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] font-semibold mt-0.5" style={{ color: brand }}>{h.price}</p>
              </div>
              <a href={`tel:${h.phone}`} className="text-[11px] font-medium flex items-center gap-1 shrink-0" style={{ color: "var(--cat-text-secondary)" }}>
      ...
      {/* Transfer */}
      <div className="rounded-2xl p-5" style={{ background: "var(--cat-card-bg)", border: "1px solid var(--cat-card-border)" }}>
        <p className="text-[10px] font-bold uppercase tracking-widest mb-4" style={{ color: "var(--cat-text-muted)" }}>Трансфер</p>
        <div className="space-y-3">
          {MOCK_TRANSFERS.map((tr, i) => (
            <div key={i} className="flex items-start gap-3 p-4 rounded-xl" style={{ background: "var(--cat-tag-bg)", border: "1px solid var(--cat-tag-border)" }}>
              <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: brand + "15" }}>
