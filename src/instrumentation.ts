...
export async function register(): Promise<void> {
  ...
  const { startNotificationDrain, startRetentionSweep, startScheduledNewsPublisher } =
    await import("./worker/notifications");
  startNotificationDrain();
  startRetentionSweep();
  // Promotes tournament_news rows from "scheduled" → "published" at publishAt
  // and fan-outs notificationQueue rows. See worker/notifications.ts.
  startScheduledNewsPublisher();
}
